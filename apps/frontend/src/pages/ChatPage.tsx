  ...
  const initSocketConnection = useCallback(
    (userInfo: UserInfo) => {
      ...
      newSocket.on("user-disconnected", (data: { socketId: string }) => {
        setUsers((prev) =>
          prev.filter((user) => user.socketId !== data.socketId)
        )
      })
              ...
              <div style={{ position: "relative" }}>
                ...
                {showAtUsers && (
                  ...
                  >
                    {users
                      .filter((user) =>
                        user.name.toLowerCase().includes(atText.toLowerCase())
                      )
                      .map((user) => (
                        <div
                          key={user.id}
                          ...
                          onClick={() => {
                            ...
                            const afterAt = inputValue.substring(
                              atPosition + atText.length + 1
                            )
