...
import { Upload, Button, Typography, Card, Progress, message, List, Space } from 'antd';
import { UploadOutlined, LeftOutlined, DeleteOutlined, PauseOutlined, PlayCircleOutlined } from '@ant-design/icons';
...
const UploadPage: React.FC = () => {
  ...
  const handleUpload = async (file: UploadFile, fileObj: File) => {
    try {
      ...
      const response = await axios.post('http://backend:3000/api/upload', formData, config);
  ...
  const uploadProps = {
    beforeUpload,
    fileList: fileList as any[], // 类型转换，解决status类型不匹配问题
    onRemove: (file: any) => {
  ...
  return (
    <div className="upload-container">
      <div className="header">
        ...
        <Title level={2} className="title">文件上传</Title>
      </div>
                      ...
                      <Space direction="vertical" size="small" style={{ width: '100%' }}>
                        ...
                        <Progress 
                          ...
                          status={
                            ...
                            file.status === 'done' ? 'success' : 'active'
                          }
                        />
                        <Text type={
                          file.status === 'error' ? 'danger' :
                          file.status === 'done' ? 'success' : 'secondary'
                        }>
